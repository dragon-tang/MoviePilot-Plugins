...
class MediaCoverGenerator(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            # 角标相关配置
            self._badge_enabled = config.get("badge_enabled")
    ...
    def __update_config(self):
        ...
        self.update_config({
            ...
            # 角标相关配置
            "badge_enabled": self._badge_enabled,
            "badge_font_url": self._badge_font_url,
            "badge_font_path": self._badge_font_path,
            "badge_font_path_local": self._badge_font_path_local,
                    ...
                    {"component": "VDivider"},
                    {
                        ...
                        "content": [
                            ...
                            },
                            {
                                ...
                                "props": {"value": "badge-tab"},
                                "content": [
                                    {"component": "VCardText", "content": badge_tab}
                                ],
                            },
                        ],
                    },
...
''',
    ...
    def __get_media_count(self, server_name, library_name):
        ...
        try:
            ...
            if not service:
                logger.warning(f"找不到服务器: {server_name}")
                return None
    ...
    def __get_fonts(self):
        ...
        # 默认角标字体
        default_badge_url = "https://raw.githubusercontent.com/siryle/MoviePilot-Plugins/main/fonts/arialbd.ttf"
        ...
        if self._badge_enabled:
            badge_font_info = {
                "lang": "角标",
                "url": self._badge_font_url or default_badge_url,
                "local_path_config": self._badge_font_path_local,
                "download_base_name": "badge",
                "hash_file_name": "badge_url.hash",
                "final_attr_name": "_badge_font_path",
                "fallback_ext": ".ttf"
            }
        ...
        for font_info in font_infos:
            ...
            if local_path_cfg:
                ...
                if self._validate_font_file(local_font_p):
                    ...
                    logger.warning(f"{lang}字体: 本地指定路径 {local_font_p} 无效或文件不存在。")
            ...
            if not using_local_font:
                ...
                if url_has_changed or not font_file_is_valid:
                    if url_has_changed:
                        logger.info(f"{lang}字体URL已更改或首次下载。")
                    if not font_file_is_valid and downloaded_font_file_path.exists():
                         logger.info(f"{lang}字体文件 {downloaded_font_file_path} 无效或损坏，将重新下载。")
                    elif not downloaded_font_file_path.exists():
                         logger.info(f"{lang}字体文件 {downloaded_font_file_path} 不存在，将下载。")
                    ...
                    if self.download_font_safely(url, downloaded_font_file_path):
                        ...
                        logger.critical(f"无法获取必要的{lang}支持字体: {url}")
                    ...
                    logger.info(f"{lang}字体: 使用已下载/缓存的有效字体 {downloaded_font_file_path}")
                    current_font_path = downloaded_font_file_path
            ...
            logger.info(f"{lang}字体最终路径: {getattr(self,final_attr)} {status_log}")
