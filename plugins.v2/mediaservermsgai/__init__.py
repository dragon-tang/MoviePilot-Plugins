...
class mediaservermsgai(_PluginBase):
    ...
    # ==================== 插件基本信息 ====================
    plugin_name = "媒体库服务器通知自用版"
    plugin_desc = "基于Emby识别结果+TMDB元数据+微信清爽版(支持排除特定库防止误识别)"
    ...
    def init_plugin(self, config: dict = None):
        if config:
            ...
            logger.info(f"插件配置已加载。排除库: {self._exclude_libs}")
        ...
        return [
            {
                ...
                'content': [
                    ...
                    {'component': 'VRow', 'props': {'show': '{{aggregate_enabled}}'}, 'content': [
                        {'component': 'VCol', 'props': {'cols': 12, 'md': 6}, 'content': [{'component': 'VTextField', 'props': {'model': 'aggregate_time', 'label': '聚合等待时间（秒）', 'type': 'number'}}]}
                    ]},
                    {'component': 'VRow', 'content': [
                        {'component': 'VCol', 'props': {'cols': 12, 'md': 6}, 'content': [{'component': 'VSwitch', 'props': {'model': 'filter_unrecognized', 'label': 'TMDB未识别视频不发送通知'}}]}
                    ]}
                ]
            }
        ], {
            "enabled": False, "types": [], "aggregate_enabled": False, 
            "aggregate_time": self.DEFAULT_AGGREGATE_TIME, "smart_category_enabled": True, 
            "filter_unrecognized": True, "exclude_libs": ""
        }
        ...
        try:
            ...
            if self._filter_unrecognized and not is_excluded:
                if event_info.item_type in ["MOV", "TV", "SHOW"]:
                    if event_type in ["library.new", "playback.start", "playback.stop", "media.play", "media.stop", "PlaybackStart", "PlaybackStop"]:
            ...
            if self._aggregate_enabled and event_type == "library.new" and event_info.item_type in ["TV", "SHOW"] and not is_excluded:
    ...
    def _process_media_event(self, event: Event, event_info: WebhookEventInfo):
        try:
            ...
            if event_info.item_type == "AUD":
                ...
                tmdb_info = None
                if tmdb_id:
                    try: tmdb_info = self.chain.recognize_media(tmdbid=int(tmdb_id), mtype=MediaType.MOVIE if event_info.item_type == "MOV" else MediaType.TV)
                    except: pass
                ...
                elif "play" in event_info.event.lower() or "start" in event_info.event.lower(): message_title = f"▶️ 开始播放：{title_name}"
                elif "stop" in event_info.event.lower(): message_title = f"⏹️ 停止播放：{title_name}"
            ...
            if "stop" in event_info.event.lower(): self._add_key_cache(expiring_key)
            if "start" in event_info.event.lower(): self._remove_key_cache(expiring_key)
            ...
            logger.error(f"媒体事件异常: {str(e)}\n{traceback.format_exc()}")

    # ==================== 辅助逻辑实现 ====================
    def _handle_test_event(self, event_info):
        self.post_message(mtype=NotificationType.MediaServer, title="🔔 媒体服务器通知测试", 
                         text=f"来自：{self._get_server_name_cn(event_info)}\n状态：连接正常", image=self._webhook_images.get(event_info.channel))
    ...
    def _handle_login_event(self, event_info):
        ...
        self.post_message(mtype=NotificationType.MediaServer, title=f"🔐 {action}提醒", 
                         text=f"👤 用户：{event_info.user_name}\n🖥️ 服务器：{self._get_server_name_cn(event_info)}", image=self._webhook_images.get(event_info.channel))

    def _handle_rate_event(self, event_info):
        curr_lib = self._get_library_name(event_info)
        tmdb_id = None if curr_lib in self._exclude_libs else self._extract_tmdb_id(event_info)
        img = self._get_tmdb_image(event_info, MediaType.MOVIE if event_info.item_type == "MOV" else MediaType.TV) if tmdb_id else None
        self.post_message(mtype=NotificationType.MediaServer, title=f"⭐ 用户评分：{event_info.item_name}", 
                         text=f"👤 用户：{event_info.user_name}\n🏷️ 标记：{self._webhook_actions.get(event_info.event, '已标记')}", image=img or self._webhook_images.get(event_info.channel))
    ...
    def _send_aggregated_message(self, series_id):
        ...
        self._append_meta_info(message_texts, tmdb_info)
        self.post_message(mtype=NotificationType.MediaServer, title=f"🆕 {title} 已入库 (含{len(msg_list)}个文件)", 
    ...
    def _merge_continuous_episodes(self, events):
        ...
        for s in sorted(season_episodes.keys()):
            ...
            if eps: res.append(f"S{str(s).zfill(2)}E{str(eps[0]).zfill(2)}-E{str(eps[-1]).zfill(2)}" if len(eps)>1 else f"S{str(s).zfill(2)}E{str(eps[0]).zfill(2)}")
        return ", ".join(res)
    ...
    def _get_tmdb_image(self, event_info, mtype):
        ...
        try:
            img = self.chain.obtain_specific_image(mediaid=event_info.tmdb_id, mtype=mtype, image_type=MediaImageType.Backdrop, 
                                                  season=event_info.season_id, episode=event_info.episode_id)
            if img: 
                if len(self._image_cache) > self.IMAGE_CACHE_MAX_SIZE: self._image_cache.pop(next(iter(self._image_cache)))
        ...
        try:
            ...
            if res.status_code == 200:
                for song in res.json().get('Items', []):
                    self._send_single_audio_notify(song, album_name, (item_data.get('Artists') or ['未知'])[0], item_data.get('Id'), item_data.get('PrimaryImageTag'), host)
        except: pass

    def _send_single_audio_notify(self, song, album, artist, cover_id, tag, host):
        texts = [f"⏰ 入库：{time.strftime('%H:%M:%S')}", f"👤 歌手：{artist}", f"💿 专辑：{album}", f"⏱️ 时长：{self._format_ticks(song.get('RunTimeTicks', 0))}"]
        img = f"{host}/emby/Items/{cover_id}/Images/Primary?maxHeight=450&maxWidth=450&tag={tag}&quality=90" if tag else None
