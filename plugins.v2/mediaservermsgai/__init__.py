...
class mediaservermsgai(_PluginBase):
        ...
        return [
            {
                ...
                'content': [
                    ...
                    {'component': 'VRow', 'content': [{'component': 'VCol', 'props': {'cols': 12}, 'content': [{'component': 'VSelect', 'props': {'multiple': True, 'chips': True, 'model': 'mediaservers', 'label': '媒体服务器', 'items': ms_items}}]}]},
                    ...
                    {'component': 'VRow', 'content': [{'component': 'VCol', 'props': {'cols': 12}, 'content': [{'component': 'VTextField', 'props': {'model': 'exclude_libs', 'label': '排除的媒体库', 'placeholder': '库名1,库名2', 'hint': '填入不希望进行 TMDB 识别的媒体库全名'}}]}]},
                    {'component': 'VRow', 'content': [
                        {'component': 'VCol', 'props': {'cols': 12, 'md': 6}, 'content': [{'component': 'VSwitch', 'props': {'model': 'aggregate_enabled', 'label': '启用入库聚合'}}]},
                        {'component': 'VCol', 'props': {'cols': 12, 'md': 6}, 'content': [{'component': 'VSwitch', 'props': {'model': 'filter_unrecognized', 'label': '未识别不通知'}}]}
                    ]}
                ]
            }
        ], {
    ...
    def send(self, event: Event):
        try:
            ...
            if "item." in event_type: self._handle_rate_event(event_info); return
    ...
    def _process_media_event(self, event: Event, event_info: WebhookEventInfo):
        try:
            ...
            expiring_key = f"{event_info.item_id}-{event_info.event}"
            if expiring_key in self._webhook_msg_keys: return
            ...
            if event_info.item_type == "AUD":
                ...
                message_title = f"{song_name} {self._webhook_actions.get(event_info.event, '通知')}"
            else:
                tmdb_info = None
                if tmdb_id:
                ...
                message_title = f"{'🆕' if 'library' in event_info.event else '▶️'} {title_name} {action}"
                ...
                if tmdb_info:
                    ...
                    if hasattr(tmdb_info, 'overview') and "library" in event_info.event:
                        ov = tmdb_info.overview
                        if len(ov) > self._overview_max_length: ov = ov[:self._overview_max_length] + "..."
    ...
    def _handle_login_event(self, event_info):
        self.post_message(mtype=NotificationType.MediaServer, title="🔐 登录", text=f"用户：{event_info.user_name}", image=self._webhook_images.get(event_info.channel))

    def _handle_rate_event(self, event_info):
        self.post_message(mtype=NotificationType.MediaServer, title=f"⭐ 评分：{event_info.item_name}", text=f"标记：{self._webhook_actions.get(event_info.event, '已标记')}", image=self._webhook_images.get(event_info.channel))
    ...
    def _extract_tmdb_id(self, event_info: WebhookEventInfo):
        ...
        if not tid and event_info.json_object:
            tid = event_info.json_object.get('Item', {}).get('ProviderIds', {}).get('Tmdb')
        if not tid and event_info.item_path:
    ...
    def _get_tmdb_image(self, event_info, mtype):
        try: return self.chain.obtain_specific_image(mediaid=event_info.tmdb_id, mtype=mtype, image_type=MediaImageType.Backdrop)
        except: return None

    def _get_category_from_path(self, path, itype, is_folder):
        if not path: return ""
        try:
            path = os.path.normpath(path)
            if is_folder and itype in ["TV", "SHOW"]: return os.path.basename(os.path.dirname(path))
    ...
    def _send_single_audio_notify(self, song, album, artist, cid, tag, host):
        texts = [f"⏰ 入库：{time.strftime('%H:%M:%S')}", f"👤 歌手：{artist}", f"💿 专辑：{album}"]
        img = f"{host}/emby/Items/{cid}/Images/Primary?maxHeight=450&maxWidth=450&tag={tag}&quality=90" if tag else None
        self.post_message(mtype=NotificationType.MediaServer, title=f"🎵 新入库音乐：{song.get('Name')}", text="\n".join(texts), image=img)
    ...
    def _add_key_cache(self, key): self._webhook_msg_keys[key] = time.time() + 600
